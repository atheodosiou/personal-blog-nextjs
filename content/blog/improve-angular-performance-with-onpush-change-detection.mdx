---
title: "Optimizing Angular Change Detection with OnPush Strategy"
description: "Learn how to optimize Angular’s change detection mechanism using the OnPush strategy. Explore best practices, performance optimizations, common pitfalls, and practical code examples."
date: 2025-03-15
readTime: "15'"
...
- **Event in a descendant of an OnPush (e.g. a button inside LoginComponent):** Suppose the event is deep in `LoginComponent`’s template (LoginComponent is OnPush). Angular will mark `LoginComponent` as well as its ancestors for check. So `LoginComponent` runs, and because it’s part of `MainComponent`’s view, Angular will also check `MainComponent` (even though `MainComponent` itself didn’t directly get the event) ([Skipping component subtrees • Angular](https://angular.dev/best-practices/skipping-subtrees)). In this cycle, everything in the tree ends up being checked except any independent OnPush subtrees not involved (in this case, `MainComponent` _was_ involved as parent; if there was another separate OnPush branch, that would be skipped).
...
Optimizing Angular’s change detection with the OnPush strategy is a powerful technique for improving application performance. By telling Angular to skip checking component subtrees unless necessary, you reduce the work done during each change detection cycle. **Key takeaways:**
...
- **OnPush Triggers:** Remember the conditions that _reenable_ checking for an OnPush component – new input references, events in its template or children, manual marks, or async pipe emissions. If none of these happen, the component can sit out the change detection dance.
- **Best Practices:** Use OnPush on components that benefit from it (often presentational components, large lists, etc.), keep your inputs immutable, and prefer using the async pipe and output events to communicate changes. Reach for `ChangeDetectorRef.markForCheck()` when you need to nudge Angular that something changed (in cases of manual data updates), and use `detectChanges()` only when you absolutely need an immediate check.
...
By following these guidelines, intermediate and advanced Angular developers can take fine-grained control over change detection and squeeze more performance out of their applications. OnPush requires you to be more intentional about how data flows through your app, but the reward is often a snappier UI and the ability to scale your app’s complexity with fewer slowdowns.

For further learning, check out the [Angular official documentation on skipping component subtrees with OnPush][angular-doc] for more examples and an in-depth explanation of Angular’s change detection behavior. Also consider exploring articles like **“Angular OnPush Change Detection - Avoid Common Pitfalls”** and **“Angular Change Detection and the OnPush Strategy”** for a deeper dive. And as always, profile and benchmark your app to ensure that any change detection optimizations are actually beneficial.

With a solid understanding of OnPush and these best practices, you’ll be well-equipped to optimize your Angular applications for better performance, only checking what truly needs to be checked. Happy coding!
